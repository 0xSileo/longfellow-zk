...
namespace proofs {
  namespace {
    ...
    TEST(ECDSA, ManualSignatureWithTamperCheck) {
      const Nat order =
          Nat("0xffffffff00000000ffffffffffffffffbce6faada7179e84f3b9cac2fc632551");
      ...
      // Your known-good signature and public key
      signature_tuple st = {
        .pkx = "0x6bf85d5fe84598b10ca6199ec09ccd7d35dcd7195fdd1dc7737a0c67006ef251",
        .pky = "0xb952f25eec1deed1fca191838990466357d314adfb7e9760b8d9afc8f39a95ad",
        .e   = "0x308258ceb599b0ef3d1e27bcfc1e1b992b376e69b0fbdca72937836a21d9c2f7",
        .r   = "0xcd1b830c4a3b929e5e9feefd0cd2442225644e0de1ccf5e3579044a418be3938",
        .s   = "0xaeec3f8ddafba90e6c8a702f6fe6ff882d3106282ae354807515199c29043534"
      };

      // --- Step 1: Check that the original signature verifies correctly ---
      {
        circuit_params in;
        prepare_witness(st, in, l, F);

        verc.verify_signature3(
            l.konst(in.pkx), l.konst(in.pky), l.konst(in.e), in.vwc);

        bool failed = ebk.assertion_failed();
        EXPECT_FALSE(failed) << "Original signature should verify but didn't.";
        if (!failed) {
          log(INFO, "Original signature PASSED as expected.");
        }
      }

      // --- Step 2: Maul one character and check that verification fails ---
      {
        signature_tuple tampered_st;
        maulSignature(st, tampered_st);  // Corrupt a hex digit

        circuit_params in;
        prepare_witness(tampered_st, in, l, F);

        verc.verify_signature3(
            l.konst(in.pkx), l.konst(in.pky), l.konst(in.e), in.vwc);

        bool failed = ebk.assertion_failed();
        EXPECT_TRUE(failed) << "Tampered signature should have failed but passed.";
        if (failed) {
          log(INFO, "Tampered signature was correctly rejected.");
        } else {
          log(ERROR, "Tampered signature was incorrectly accepted!");
        }
      }
    }
  }  // namespace
